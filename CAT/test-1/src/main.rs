...
fn main() {
    ...
    for _ in 0..100 {
        ...
        if med.trim() == "Alzheimer" && age.trim().parse::<i32>().unwrap() > 50 && child.trim().parse::<i32>().unwrap() > 4 && vil.trim() == "Akpabom"
        {
            discount = a1 - (a1 * 20 / 100);
        } 
        else if med.trim() == "Arrhythmia" && age.trim().parse::<i32>().unwrap() == 30 && sib.trim().parse::<i32>().unwrap() > 4 && vil.trim() == "Ngbauji"
        {
            discount = a2 - (a2 * 5 / 100);
        } 
        else if med.trim() == "Chronic Kidney Disease" && age.trim().parse::<i32>().unwrap() > 40 && sib.trim().parse::<i32>().unwrap() > 3 && child.trim().parse::<i32>().unwrap() > 3 && vil.trim() == "Atabrikang"
        {
            discount = a3 - (a3 * 15 / 100);
        } 
        else if med.trim() == "Diabetes" && age.trim().parse::<i32>().unwrap() > 28 && age.trim().parse::<i32>().unwrap() < 45 && child.trim().parse::<i32>().unwrap() >= 2 && child.trim().parse::<i32>().unwrap() <= 4 && vil.trim() == "Okorobilom"
        {
            discount = a4 - (a4 * 10 / 100);
        } 
        else if med.trim() == "Arthritis" && age.trim().parse::<i32>().unwrap() > 58 && sib.trim().parse::<i32>().unwrap() > 5 && child.trim().parse::<i32>().unwrap() > 5 && vil.trim() == "Emeremen"
        {
